...
int main(){//takeinput();
    ...
    while(TC--){
        ...
        V=V*V;
        W=W*W;
        float64 ans = (4*u*v*w - u*(v+w-U)*(v+w-U) - v*(w+u-V)*(w+u-V) - w*(u+v-W)*(u+v-W) +(v+w-U)*(w+u-V)*(u+v-W));
        //float64 ans = (4*u*v*w - u*(v+w-U)*(v+w-U) – v*(w+u-V)*(w+u-V) - w*(u+v–W)*(u+v-W) + (v+w–U)*(w+u–V)*(u+v–W) )/12;
        ans = sqrt(ans);
        ans/=12.0;
        //ans= round(ans*10000);
        //ans/=10000;
