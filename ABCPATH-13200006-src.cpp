/*
   Author : SKRCODE
*/

#include <vector>
#include <list>
#include <map>
#include <set>
#include <deque>
#include <queue>
#include <stack>
#include <bitset>
#include <algorithm>
#include <functional>
#include <numeric>
#include <utility>
#include <sstream>
#include <fstream>
#include <iostream>
#include <iomanip>
#include <cstdio>
#include <cmath>
#include <cstdlib>
#include <cctype>
#include <string>
#include <cstring>
#include <string.h>
#include <cassert>
#include <climits>

using namespace std;

const int oo = int(2e9) + 9;
const double eps = 1e-9;
const double pi = 3.14159265358979323846264338327950;

#define debug printf("ERROR DETECTED...!!\n");
#define debug1(p)        cerr << #p << ": " << p << endl;
#define debug2(p, q)     cerr << #p << ": " << p << " | " << #q << ": " << q << endl;
#define debug3(p, q, r)  cerr << #p << ": " << p << " | " << #q << ": " << q << " | " << #r << ": " << r << endl;

#define  takeinput() freopen("ip.txt","r",stdin);
#define  takeoutput() freopen("op.txt","w",stdout);

typedef vector < int > vi;
typedef pair < int, int> ii;
typedef vector < ii > vii;

#define trvi(c, it) for (vi::iterator it = (c).begin(); it != (c).end(); it++)
#define trvii(c, it) for (vii::iterator it = (c).begin(); it != (c).end(); it++)
#define pb push_back
#define mp make_pair


typedef long long int64;
typedef double float64;

//START
char grid[100][100];
int rc[] = {-1,0,1};
int memo[100][100];
int dfs(int row,int col,int n,int m) {
    if(row>n || row<1 || col<1 || col>m)return 0;
    if(memo[row][col]!=-1)return memo[row][col];
    int ans = 0;
    for(int i=0;i<3;i++)
        for(int j=0;j<3;j++)
            if(grid[row+rc[i]][col+rc[j]] == grid[row][col]+1)ans = max(ans,1+dfs(row+rc[i],col+rc[j],n,m));
    return memo[row][col] = ans;
}

int main() { //takeinput();
    int n,m;
    int kase = 1;
    while(1) {
        int n,m;
        scanf("%d %d",&n,&m);
        if(!n && !m) break;
        cin.ignore();
        memset(grid,0,sizeof(grid));
        for(int i=1;i<=n;i++) {
            for(int j=1;j<=m;j++) scanf("%c",&grid[i][j]);
            cin.ignore();
        }

        memset(memo,-1,sizeof(memo));
        int ans = 0;
        for(int i=1;i<=n;i++)
            for(int j=1;j<=m;j++)
                if(grid[i][j] == 'A')ans = max(ans,1+dfs(i,j,n,m));

        printf("Case %d: %d\n",kase++,ans);
    }
    return 0;
}
